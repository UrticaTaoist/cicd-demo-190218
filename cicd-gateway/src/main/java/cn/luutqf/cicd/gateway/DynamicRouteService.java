...
//import static cn.luutqf.cicd.gateway.RedisRouteDefinitionRepository.GATEWAY_ROUTES;
...
//    }
    ...
    public void main() {
        ...
        definition.setId("id");
//        URI uri = UriComponentsBuilder.fromHttpUrl("lb://cicd-demo").build().toUri();
        URI uri = null;
        try {
            uri = new URI("lb://cicd-demo");
        ...
        redisTemplate.opsForHash().put(CommonConstants.ROUTE_KEY, "key", JSON.toJSONString(definition));
    }
    @PostConstruct
    public void main2() {
        RouteDefinition definition = new RouteDefinition();
        definition.setId("id2");
//        URI uri = UriComponentsBuilder.fromHttpUrl("lb://cicd-demo").build().toUri();
        URI uri = null;
        try {
            uri = new URI("lb://cicd-demo");
        ...
        Map<String, String> predicateParams = new HashMap<>(8);
        predicateParams.put("pattern", "/jd2");
        predicate.setArgs(predicateParams);
        ...
        System.out.println("definition:" + JSON.toJSONString(definition));
        redisTemplate.opsForHash().put(CommonConstants.ROUTE_KEY, "dsf", JSON.toJSONString(definition));
    }
}
